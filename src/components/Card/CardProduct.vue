...
<template>
  <div row justfy-center min-h-screen overflow-hidden px-5 mx-auto lg="pt-24" sm="pt-15">
    <div v-for="(product, id) in props.products" :key="id" sm="col-12" md="col-6 flex grow-1" lg="col-3 flex grow-1">
      <a :href="`/product/${product.id}`" w-full my-3 mx-2 p-4 flex flex-col justify-between shadow-md rounded-4 dark="bg-darksecondary">
        <div>
          ...
          <div class="max-w-[80%] font-bold" text-gray>{{ product.title }}</div>
          <div font-extrabold f-26 my-1>${{ product.price }}</div>
          <div f-10 capitalize my-1 rounded-4 bg-lightgray text="gray" text-center px-2 py-1 whitespace-nowrap inline-flex>{{ product.category }}
          </div>
        </div>
        <div py-2>
          <div justify-between flex items-center f-14 text-gray>
            ...
            <button text-white bg-dark py-1 px-4 rounded-4 f-14 flex items-center><i i-mdi-cart-plus inline-flex /><span ml-1 font-bold />Add Cart</button>
          </div>
        </div>
      </a>
    </div>
